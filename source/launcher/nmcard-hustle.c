...
static arg arglist[] = {
    { 0, "-h", "--help", 0,
      "Show help message\n" },
    { 1, "-A", "--all-boards", 0,
      "Use all boards (disabled by default) (equivalent '-b All')\n" },
    { 2, "-b", "--board", 1,
      "Specify board to use (0 by default), integer index or 'All' (equivalent '-A')\n" },
    { 6, "-r", "--reset", 0,
      "Only reset board(s)\n" },
    { 10, "-dw", "--dont-wait", 0,
      "Launch board part and leave it running infinitely\n\t\t\t\tuntil someone resets "
      "board or something else\n" },
    { 12, "-t", "--timeout", 1,
      "Time in seconds that program will running (300 by default)\n\t\t\t\tHas no effect "
      "if '-dw' option is present\n\t\t\t\tIf zero runs infinitely\n\t\t\t\t\n" },
    { 14, "-d", "--debug", 0,
      "Produce some debug output\n" },
    { 16, "-noio", "--dont-use-io", 0,
      "Do not pass output from device\n" }
};
...
int main(int argc, char *argv[])
{
    ...
    if (boards == NULL) {
        DBG("DBG: Memory allocation fail!\n");
        exit(RET_ERROR);
    }
    ...
    for (int b = 0; b < boardCount; b++) {
        ...
        for (int i = 0; i < 16; i++) {
            ...
            //nmc get access & load programs
            boards[b].nm[i].core.cluster_id = i / 4;
            boards[b].nm[i].core.nm_id = i % 4;

            if (PL_GetAccess(boards[b].board, &boards[b].nm[i].core,
                             &boards[b].nm[i].access) != PL_OK) {
            ...
            if (PL_LoadProgramFile(boards[b].nm[i].access, NMC_BIN) != PL_OK) {
                ...
                DBG("DBG:\t[%d.%d]: Program loaded\n", boards[b].nm[i].core.cluster_id,
                    boards[b].nm[i].core.nm_id);
            }
        }
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (PL_GetAccess(boards[b].board, &boards[b].cluster[i].core,
                             &boards[b].cluster[i].access) != PL_OK) {
                fprintf(stderr, "ERROR: Board %d [%d.%d]: Failed get access!\n", b,
                        boards[b].cluster[i].core.cluster_id,
                        boards[b].cluster[i].core.nm_id);
                close_everything_and_exit(RET_ACCESS_FAIL);
            }
            ...
            if (PL_LoadProgramFile(boards[b].cluster[i].access, ARM_BIN) != PL_OK) {
                fprintf(stderr, "ERROR board %d [%d.%d]: Failed load program '%s'\n", b,
                        boards[b].cluster[i].core.cluster_id,
                        boards[b].cluster[i].core.nm_id, ARM_BIN);
                close_everything_and_exit(RET_LOAD_PROGRAM_FAIL);
            } else {
                DBG("DBG:\t[%d.%d]: Program loaded\n",
                    boards[b].cluster[i].core.cluster_id,
        ...
        if (PL_GetAccess(boards[b].board, &boards[b].central.core,
                         &boards[b].central.access) != PL_OK) {
            fprintf(stderr, "ERROR: Board %d [%d.%d]: Failed get access!\n", b,
        ...
        if (PL_LoadProgramFile(boards[b].central.access, CARM_BIN) != PL_OK) {
            ...
            DBG("DBG:\t[%d.%d]: Program loaded\n", boards[b].central.core.cluster_id,
                boards[b].central.core.nm_id);
        }

        if (use_io) {
            int io_err = 0;
            boards[b].central.io =
                IO_ServiceStart(boards[b].central.access, NULL, NULL, &io_err);

            if (boards[b].central.io == NULL) {
                fprintf(stderr,
                        "ERROR board %d [%d.%d]: Failed create IO_Service: IO_err = %d\n",
                        b, boards[b].central.core.cluster_id,
                        boards[b].central.core.nm_id, io_err);
    ...
    printf("[Launced]\n");
    // Are we need to stay here while board running, or just launch it and exit?
    if (wait_result) {
...
int help_message(int retValue)
{
    ...
    for (int i = 0; i < args_total; i++) {
        printf("\t%s/%s %s\t: %s\n", arglist[i].shrt, arglist[i].lng,
               arglist[i].req_val ? "[value]" : "", arglist[i].help_str);
    }
    exit(retValue);
}
...
{
    for (int i = 0; i < args_total; i++) {
        if ((strcmp(arg_str, arglist[i].shrt) == 0) ||
            (strcmp(arg_str, arglist[i].lng) == 0)) {
    ...
    for (int arg_pos = 1; arg_pos < argc; arg_pos++) {
        ...
        case 2:
            ...
            if ((parse_int(argv[arg_pos], &int_value) != 0) &&
                (strcmp(argv[arg_pos], "all") && strcmp(argv[arg_pos], "All") &&
                 strcmp(argv[arg_pos], "ALL"))) {
                fprintf(stderr,
                        "Invalid optiov value: %s %s\nMust be positive integer or "
                        "'All'.\n\n",
                        argv[arg_pos - 1], argv[arg_pos]);
                help_message(RET_WRONG_ARG);
            }
            if (!strcmp(argv[arg_pos], "all") || !strcmp(argv[arg_pos], "All") ||
                !strcmp(argv[arg_pos], "ALL")) {
                all_boards_in_use = 1;
            } else {
